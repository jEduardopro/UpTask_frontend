...
const Search = () => {
	...
	const projectsFiltered = search === '' ? [] : projects.filter(project => {		
		return JSON.stringify(Object.values(project).join(' ')).toLowerCase().includes(search.toLowerCase())
	})
			...
			<Dialog as="div" className="fixed inset-0 z-10 overflow-y-auto mt-20 p-4 sm:p-20 md:p-20" onClose={handleSearcher}>
				...
				>
					<Combobox
						...
						onChange={ (project: Project) => (project && window.location.replace(`/projects/${project._id}`))}
					>
